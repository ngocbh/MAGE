...
class TrainModel(object):
    ...
    def train(self, train_params=None, optimizer_params=None):
        ...
        for epoch in range(num_epochs):
            ...
            if best_eval_acc < eval_acc or (best_eval_acc == eval_acc and best_eval_loss > eval_loss):
                is_best = True
...
def main(config):
    ...
    ## saving test results
    saving_fp = os.path.join('checkpoints', 'test_acc.json')
    if os.path.isfile(saving_fp):
        with open(saving_fp, 'r') as f:
            test_acc_dict = json.load(f)
    else:
        test_acc_dict = {}

    if config.models.gnn_name not in test_acc_dict:
        test_acc_dict[config.models.gnn_name] = {}
    if config.datasets.dataset_name not in test_acc_dict[config.models.gnn_name]:
        test_acc_dict[config.models.gnn_name][config.datasets.dataset_name] = {}

    test_acc_dict[config.models.gnn_name][config.datasets.dataset_name] = {
        'test_acc': test_acc,
        'test_loss': test_loss
    }
    with open(os.path.join('checkpoints', 'test_acc.json'), 'w') as f:
