...
def scores2coalition(scores, cutoff=None, sparsity=None):
    assert sparsity is not None or cutoff is not None
...
def evaluate_coalition(explainer, data, coalition, subgraph_building_method='remove'):
    subgraph_building_func = get_graph_build_func(subgraph_building_method)
    ...
    if len(coalition) == num_nodes:
        ...
        mask[coalition] = 1.0
        masked_x, masked_edge_index = subgraph_building_func(
        ...
        maskout_x, maskout_edge_index = subgraph_building_func(
